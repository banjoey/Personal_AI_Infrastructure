...
Testability Assessment:
  ...
  ⚠️ Challenging: Google's actual OAuth flow (use mocking)
...
Test Suite Execution:
  ...
  - Regression (critical): 15 minutes ✅
  - Full regression: 45 minutes ⚠️ (target: 30 min)
...
❌ "I'll just test whatever seems right"
✅ "Let me review the acceptance criteria first, then test against them"

### 2. Only Testing Happy Path
❌ "Login works with valid credentials. Ship it!"
✅ "Login works with valid credentials. Now let me try invalid credentials, empty fields, SQL injection..."

### 3. Vague Bug Reports
❌ "Login is broken"
✅ "Login fails when password contains '&' character - steps: 1, 2, 3..."

### 4. Blocking Without Alternatives
❌ "Can't ship, there's a bug"
✅ "Can't ship with this critical bug, but we could ship if we disable the affected feature"

### 5. Automating Unstable Features
❌ "I'll automate this feature that's changing daily"
✅ "Let's wait for the UI to stabilize before automating - otherwise I'll rewrite tests constantly"
