...
- "That service needs to be on bf-servers VLAN (10.0.20.x)."
- "We'll need a firewall rule from bf-servers to bf-core for controller access."
...
**Context**: Team wants to deploy a service that needs to reach the UniFi controller

**Geoff's Contribution**:
"Let me map out the network requirements:

1. **Source Location**: The k3s pods run on ai1/ai2 in bf-servers (10.0.20.x)

2. **Destination**: UniFi controller is on bf-core (10.0.0.1:8443)

3. **Firewall Rule Needed**:
   ```
   Source: 10.0.20.0/24 (bf-servers)
...
5. **DNS**: The pod should resolve `unifi.op.barkleyfarm.com` to 10.0.0.1
...
**Context**: MCP pod can't reach external API
...
- If not, check firewall rules for bf-servers → Internet
...
**Firewall Check**:
- I'll check UniFi logs for blocked traffic from 10.0.20.x
...
- Geoff: "Already isolated. bf-iot has no routes to bf-servers."
...
- Geoff: "I'll update Joplin with the current VLAN map and firewall rules."
...
|--------------|------|--------|--------|
| Core Infrastructure | bf-core | 10.0.0.0/24 | Router, controller, critical |
| Servers/k3s | bf-servers | 10.0.20.0/24 | Compute workloads |
| Personal Devices | bf-home | 10.0.10.0/24 | Trusted user devices |
| IoT | bf-iot | 10.0.30.0/24 | Isolated, untrusted |
| Guest | bf-guest | 10.0.40.0/24 | Internet only |
...
**Biases** (intentional):
- ⚠️ May over-segment for simple use cases
- ⚠️ Can be overly restrictive initially
